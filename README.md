## In the GitHub application repository, if a developer makes changes, it triggers a CI (Continuous Integration) job in Jenkins. This job pulls the code from the application repository, builds and tests artifacts using Maven, and performs static code analysis using SonarQube. It then builds a Docker image and pushes it to Docker Hub. The Docker image is scanned using Trivy. Once the CI job is completed, a CD (Continuous Deployment) job is automatically triggered. The CD job updates the build number in the deployment YAML file in the GitHub repository. Using ArgoCD, it pulls the Kubernetes manifest files from the GitHub repository and deploys the resources on the Amazon Elastic Kubernetes Service (EKS) cluster.
